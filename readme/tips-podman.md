...
In some situations, as inside the container the data has been generated **as root**, we are not able to **remove** the data **outside** it. But in some cases, such as the **mongodb** service, the data must be removed with the container **stopped**, so this workaround allows to remove data using root inside a container:
